...
RSpec.describe WorkoutCommentsController, type: :controller do
  let!(:user) { create(:user) }
  let!(:workout) { create(:workout) }
  let(:params) { { workout_comment_id: workout_comment.id } }

  let(:create_workout_comment) do
    allow_any_instance_of(WorkoutCommentsController).to receive(:current_user).and_return(user)
